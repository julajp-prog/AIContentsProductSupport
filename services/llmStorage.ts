...
export const INITIAL_PROVIDERS_CONFIG: Record<ProviderType, LLMProviderConfig> = {
  ...
  lmstudio: {
    ...
    description: 'LM Studioのローカル推論サーバー（OpenAI互換）。Direct(直接)とProxy(プロキシ)の双方で確実通信＆徹底JSON崩れ修復。',
  ...
  lmstudio_bionic: {
    ...
    selectedModel: 'local-model',
    availableModels: ['local-model'],
    description: 'LM Studio Bionicプリセット。低遅延ストリーミングと高速推論・厳格JSONフォーマット検証に対応。',
    docsUrl: 'https://lmstudio.ai',
    testStatus: 'untested',
    jsonMode: 'auto',
    autoRepairJson: true,
  ...
  unsloth: {
    id: 'unsloth',
    name: 'Unsloth Studio (推論サーバー)',
    category: 'local',
    enabled: false,
    baseUrl: 'http://localhost:8000/v1',
    connectionMode: 'direct',
    proxyUrl: '/api/proxy/unsloth',
    defaultEndpoint: 'http://localhost:8000/v1',
    selectedModel: 'unsloth-lora-model',
    availableModels: ['unsloth-lora-model', 'meta-llama-3.1-8b', 'qwen2.5-coder-7b'],
    description: 'UnslothでファインチューニングされたLoRA/GGUF/vLLMローカル推論サーバー。JSON崩れ徹底防止・自動修復。',
  ...
  openai_compat: {
    id: 'openai_compat',
    name: 'OpenAI互換 (vLLM / Jan / LocalAI)',
    category: 'local',
    enabled: false,
    baseUrl: 'http://localhost:8000/v1',
    connectionMode: 'direct',
    proxyUrl: '/api/proxy/openai-compat',
    defaultEndpoint: 'http://localhost:8000/v1',
    selectedModel: 'default',
    availableModels: ['default'],
    description: 'vLLM, Jan, LocalAI, Text-Gen-WebUIなど標準OpenAI互換APIエンドポイント。JSON Mode適合性チェック対応。',
    docsUrl: '',
    testStatus: 'untested',
    jsonMode: 'auto',
    autoRepairJson: true,
